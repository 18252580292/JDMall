...
import com.jskj.jdmall.util.ActivityUtils
import com.jskj.jdmall.util.StringUtils
import com.jskj.jdmall.util.toast
import com.orhanobut.logger.Logger
import java.lang.ref.WeakReference

class SplashActivity : BaseActivity(), View.OnClickListener, ModeChangedListener {
        ...
        override fun handleMessage(msg: Message?) {
            when(msg?.what) {
                MessageAction.MSG_LOGIN_SUCCESS -> {
                    ...
                    val loginActivity = wtf.get() as SplashActivity
                    if(loginBean.success) {
                        ActivityUtils.start(loginActivity, MainActivity::class.java, true)
                    } else {
